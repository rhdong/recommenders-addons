...
class HorovodTest(test.TestCase):
  ...
  def common_minimize_trainable(self, base_opt, test_opt, name):
    ...
    hvd.init()
    test_opt = de.DynamicEmbeddingOptimizer(test_opt, horovod_synchronous=True)
    for dtype, run_step, dim in itertools.product([dtypes.float32], [2], [10]):
      ...
      base_opt_op = base_opt.minimize(base_loss, global_step)
